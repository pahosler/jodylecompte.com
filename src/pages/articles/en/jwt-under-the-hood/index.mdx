...
micro-library needs to be able to accomplish. For the sake of brievity we are going to focus on the following
...
way of building simple Node apps, so I am going to skip that portion, but if you are unfamilar with building a
...
from a string or a Base64 encoded string and outputting either making for a convinient way to handle our encoding.
...
After we split our token and destruture it into its three pieces, we can go ahead and decode our header and payload.
At this point, we aren't actually sure if the token is even trust-worthy, it's possible it's been tampered with. But
we need to know if there is an expiration property on the claim to ensure it's not expired. Additionlly, if this
were a more featre-packed library, we would need the `alg` property off of the header to know which algorithm to use
...
key back into `generateSignature`. If the resulting signature does not match the signature we recieve, the token
...
2. The expiration is ommitted so that we know for certain the token will be valid and not expired.
...
I definitely did! It's not often that you could read the full RFC for a standard and write a barebones implementation
