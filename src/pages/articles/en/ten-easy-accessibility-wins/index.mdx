...
things significantly more accessible without using any additional time. I am not advocating for avoiding this reality, I'm hoping to encourage
people by showing them how easy it can be to get started with a few first steps.

## 1. Install Accessibility Testing Tools

While not technically an "easy win", it's an important step one to ensure you don't go into battle without your sword or shield. Accessibility
testing tools are key in helping you catch overlooked or missed problems as close to them occurring as possible. The more work 
that is completed before these issues get fixed, the longer it will take to implement even basic accessibility.

A few of my favorites:

- [WAVE](https://wave.webaim.org/) - A powerful, free browser extension for identifying almost everything, if not more than the built-in browser developer
  tools can detect. I use wave to run through any touched pages before each merge request to ensure the small and easy details are taken care of first.
- [aXe DevTools](https://www.deque.com/) - A comprehensive suite of tools ranging in feature parity with other tools in their free tier to 
...
- [EsLint](https://eslint.org/) - If you are working in JavaScript, you are probably already using ESLint. You can also use custom rule packages
  ...
  in real time as you are coding. If you are particularly masochistic like me, then you can also configure your setup to fail builds due to
  accessibility issues. If you are not a React developer, or not working heavily using JavaScript, I'm sure there are similar tools for your stack of
  choice.
...
doing audits, it is very rare to encounter a website that doesn't have a few missing alt tags. If you are conveying important
information in the means of an image without an alt tag, then non-visual users are left out.

To the credit of the average programmer, I see this far more often with icons than regular images. Depending on your needs,
there are multiple ways you can attack this problem.

If an image doesn't convey any meaningful information just hide it. For example, the images featured at the top of each of my blog posts
have the exact same text (more or less) as the article headline, so there is no real value to a screen reader reading it to the user twice.
...
In the case of icons, my personal preference is to use a wrapper component that adds `aria-hidden` to the icon a text label
that is hidden from visual users. The following screen reader only class is my go-to, written by [Chris Coyier](https://twitter.com/chriscoyier) of
...
Alternatively, you can just add a `title` or `aria-label` to your icon. I dislike this method because some screen readers will still
announce that an image is there when communicating information is the more important behavior. More on this soon in [#6](#6-use-anchors-and-buttons-appropriately) and [#9](#9-avoid-color--imagery-as-the-sole-medium-for-information)

## 3. Responsiveness for Zoom

With the idea of "mobile first" being a decade old at this point, I think most developers are definitely considering mobile responsiveness
...
all your typography to scale with these changes, but special care is needed with media queries.
...
they are now effectively working with 750 pixels. A mobile or tablet view, rahter than a desktop view, may increase usability.

While there are some simple solutions, it is a surprisingly in-depth topic. For a deeper understanding of
...
above will detect insufficient contrast. [Fontanello](https://fontanello.app/) is one of my favorite tools for this because I can quickly highlight a piece
...
The thing that makes this hard is from a design standpoint, and also from a client facing standpoint, is companies can be very passionate about their
...
Keyboard navigation in web user interfaces is a big one. Not only does it allow people who may only use a keyboard, and not a mouse for
any number of reasons, but also power users that see reaching for the mouse as a capital offense.

### Use the Platform When You Can

Most of the time, we get that keyboard navigation for free by making use of the platform. For example, given a regular old datepicker input:
...
never use third-party or always use the platform, development is nuanced. But it is something to be aware of, I dive a bit deeper into this in number [10](#10-research-third-party-components).
...
often a disproportionate amount of attention placed on the idea that users are using a combination of a screen reader **and** using their
keyboard only, but there are also sighted users who prefer using the keyboard for any number of potential reasons. They miss out
...
JavaScript driven front-ends increased the number of mishaps in this regard.
...
A screen reader will not be aware of what the image is or that it is click enabled, it will simply read "image". Instead consider,
...
infer what action will be produced. To re-iterate on some earlier, this is a good example of my "accessible-first development" idea, because
...
Opinions may vary, but I believe that forms may be the second most important thing on the internet after content.
...
Incorrect form lables are responsible for the majority of form input issues I encounter. Common form input lable issues are, having a telephone or 
similar icon next to them, placeholder labels, or a visual label that is not programmatically associated
with it's respective input. There is a lot of flexibility here, and I've never noticed a difference in interpretation by assistive technology, 
...
I'm partial to option 2, however there are benefits to other approaches. For example, using `aria-labeledby` to combine multiple labels together into
a singular accessible label.

### Keyboard Control

Re-visiting [#5](#5), the form should be able to be navigated backwards, forwards, and submitted with
a keyboard. The platform, unless we deviate,
will typically address this for us. The biggest thing to watch out for that I see
broken most often, especially on JavaScript driven forms, is ensuring that the
submit button can be triggered via enter while focused on a form
element. Pressing enter prematurely may require error handling.
...
HTML5 validation properties when it makes sense. For example, if your input has the required property, then a screen reader will 
announce that it is a required input when it reads its label.

HTML5 validation by default will also announce to the user that the validation has failed, letting them know what went wrong. This is a 
deep topic, but an easy first step is ensure the area where your error is rendered has the ARIA role of "alert" so assistive
technology will inform the user.

Errors should always be clear in their nature. Text simply being highlighted red or outlining a form field is not sufficient by its self. More on that soon.

## 8. Test with an Actual Screen Reader

This one is short and sweet. As mentioned earlier, automated testing tools like WAVE or aXe can only catch certain classes errors. To ensure
the best experience possible for screen reader users, you need to test your site with a screen reader. The litmus test I ask of
people is to pull up their website or latest project, turn on their screen reader, close their eyes, and attempt to locate and submit the contact form.
...
Using color or images as the sole mechanism by which information is conveyed to the user has some assumptions that are not 
...
available outside of those. In the case of form errors, you can show your text in red or orange, or change the outline color of the
input field, but there should also be a clear and concise text indication of what the error is as well.

Just be mindful of the point of diminishing returns. For example, if you give an image an alt tag with **too much** information then it
...
Some components are harder to get right from an accessibility standpoint. There are some components, like a date picker, that I don't even feel
...
in a profound way is to stand on the shoulder of giants.

There are frameworks like [React UI](https://reach.tech/) and [Shoelace](https://shoelace.style/) where considerable amounts of thought, energy, and 
a fair amount of sweat and tears went into making components that are as accessible as possible. If you are unsure about a particular framework
or unsure how to best audit the accessibility of a UI kit before committing, feel free to ask the community. A lot of us spend a considerable amount
...
a hour to transition over and retire my old menu.
...
understanding of the whens and whys, peel back the next layer of the onion to level up your skill and make your work even more accessible.
