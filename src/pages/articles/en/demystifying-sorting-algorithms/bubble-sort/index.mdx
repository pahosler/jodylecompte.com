...
because the implementation is trivial to implement.
...
like bubble sort unsuitable for large datasets.
...
to fully sort, it isn't used for actual index lookups in the input. But take note of the inner loop. The terminating
...
The above implementation will always be the worst case of O(N^2), because there is no safe-guard
against unnecessary iterations. For example, the first sample was pretty mixed up, but what if our array was mostly already
sorted.
...
that if no swaps happened in the last inner loop we can return the list as sorted.
...
don't always have a consistent runtime complexity. With this optimization, we can now achieve
